...
public class RedisDemoApplication {
    ...
    public static void main(String[] args) {
        ...
        while(true) {
            System.out.println("键入操作");
            System.out.println("1)count自增  \t\t2)count增加n");
            System.out.println("3)显示n秒内频率\t\t4)重置数据库");
            ...
            try {
                switch (str) {
                    case "1":
                        ...
                        jedisUtil.addLog(System.currentTimeMillis());
                        System.out.println("操作完成。count 的值为" + jedisUtil.get("count"));
                        break;

                    case "2":
                        System.out.println("请输入想要增加的数值");
                        System.out.print(">>>");
                        scan = new Scanner(System.in);
                        str = scan.next();
                        try {
                            int incnum = Integer.parseInt(str);
                            jedisUtil.incr("count", incnum);
                            jedisUtil.addLog(System.currentTimeMillis());
                            System.out.println("操作完成。count 的值为" + jedisUtil.get("count"));
                        }
                        catch (Exception e) {
                    ...
                    case "3":
                        ...
                        try {
                            ...
                            System.out.println("这段时间FREQ 的值为" + jedisUtil.getFreq(num*1000));
                        } catch (Exception e) {
                            System.out.println("非法输入");
                        }
                        break;
